...
// .wrangler/tmp/bundle-h6DWS6/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-h6DWS6/middleware-insertion-facade.js
init_modules_watch_stub();
...
async function htmlPage(session, version, pageUrl) {
  const qrSvg = await import_qrcode.default.toString(pageUrl, { type: "svg", margin: 2, width: 160 });
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
var src_default = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/upload" && request.method === "POST") {
      ...
      const text = String(form.get("text") || "").slice(0, 2e3);
      const contact = String(form.get("contact") || "").slice(0, 256);
      const discordForm = new FormData();
      const contentParts = [];
      contentParts.push(`\u{1F399}\uFE0F Voice note from kimiflare v${version}`);
