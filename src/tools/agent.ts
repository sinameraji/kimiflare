...
export function makeAgentTool(allowedTypes: SubagentType[] = ALL_TYPES): ToolSpec<AgentArgs> {
  ...
  return {
    ...
    description: [
      ...
      "",
      "PARALLEL DISPATCH (code mode): compose with Promise.all to run",
      "multiple children concurrently in a single script. Each runs with",
      "its own isolated context; you get back their summaries together:",
      "",
      "  const [auth, session, gate] = await Promise.all([",
      "    api.Agent({ subagent_type: \"explore\", description: \"find auth files\",     prompt: \"...\" }),",
      "    api.Agent({ subagent_type: \"explore\", description: \"find session handling\", prompt: \"...\" }),",
      "    api.Agent({ subagent_type: \"explore\", description: \"find access gates\",     prompt: \"...\" }),",
      "  ]);",
      "  console.log(`auth: ${auth}\\nsession: ${session}\\ngate: ${gate}`);",
      "",
      "This is the natural parallel-execution surface in code mode — much",
      "more efficient than reading files inline if the sub-investigations",
      "are independent, because each child keeps its own context lean.",
      "",
